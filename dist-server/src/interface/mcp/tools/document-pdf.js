...
exports.registerGenerateDocumentPdfTool = registerGenerateDocumentPdfTool;
 ...
 */
async function addImagePages(doc, imagePath) {
    ...
    if (w > 0 && h > 0 && w > h * LANDSCAPE_RATIO) {
        ...
        const leftBuffer = await (0, sharp_1.default)(imagePath)
            .extract({ left: 0, top: 0, width: halfWidth, height: h })
            .toBuffer();
        const rightBuffer = await (0, sharp_1.default)(imagePath)
...
function registerGenerateDocumentPdfTool(server) {
    server.registerTool("generate-document-pdf", {
        description: "Generates a complete PDF from chapter image folders. " +
        ...
        inputSchema: zod_1.z.object({
            chapters: zod_1.z
                .array(zod_1.z.number().int().min(1).max(9))
                .optional()
            ...
            mode: zod_1.z
                ...
                .optional()
                .describe("'images' (default): original images full-page, faithful rendering. " +
        ...
        const doc = new pdfkit_1.default({ size: "A4", margins: defaultMargins, autoFirstPage: false });
        ...
        const worker = needsOcr ? await (0, tesseractWorker_1.getTesseractWorker)(ocrLanguage) : null;
        ...
        for (const chapterNum of chapterList) {
            ...
            for (const imageFile of imageFiles) {
                ...
                if (needsVision) {
                    ...
                    let visionText = "";
                    try {
                        visionText = await (0, githubVisionService_1.extractTextFromImage)(imagePath, "francais");
                    }
                    catch (err) {
