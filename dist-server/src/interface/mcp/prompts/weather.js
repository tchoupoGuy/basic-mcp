...
exports.registerWeatherPrompts = registerWeatherPrompts;
const zod_1 = require("zod");
function registerWeatherPrompts(server) {
    server.registerPrompt("analyze-weather", {
        description: "Generate a prompt to analyze the weather for a city and give actionable advice",
        argsSchema: {
            city: zod_1.z.string().describe("City name to analyze weather for"),
            language: zod_1.z.enum(["fr", "en"]).default("fr").describe("Response language"),
        },
    }, ({ city, language }) => {
    ...
    server.registerPrompt("compare-weather", {
        description: "Generate a prompt to compare the weather between two cities",
        argsSchema: {
            city1: zod_1.z.string().describe("First city"),
            city2: zod_1.z.string().describe("Second city"),
        },
    }, ({ city1, city2 }) => ({
        messages: [
            {
